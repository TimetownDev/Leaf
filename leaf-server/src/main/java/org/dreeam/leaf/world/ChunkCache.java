...
public final class ChunkCache<V> {
    ...
    public ChunkCache(Thread thread, final int expected, final float f) {
        ...
        this.f = f;
        minN = n = it.unimi.dsi.fastutil.HashCommon.arraySize(expected, f);
        mask = n - 1;
        maxFill = it.unimi.dsi.fastutil.HashCommon.maxFill(n, f);
        key = new long[n + 1];
    ...
    private V removeEntry(final int pos) {
        ...
        size--;
        shiftKeys(pos);
        if (n > minN && size < maxFill / 4 && n > it.unimi.dsi.fastutil.Hash.DEFAULT_INITIAL_SIZE) {
            rehash(n / 2);
        }
        return oldValue;
    }
    ...
    private V removeNullEntry() {
        ...
        value[n] = null;
        size--;
        if (n > minN && size < maxFill / 4 && n > it.unimi.dsi.fastutil.Hash.DEFAULT_INITIAL_SIZE) rehash(n / 2);
        return oldValue;
    }
    ...
    private void insert(final int pos, final long k, final V v) {
        ...
        key[pos] = k;
        value[pos] = v;
        if (size++ >= maxFill) rehash(it.unimi.dsi.fastutil.HashCommon.arraySize(size + 1, f));
    }
}
